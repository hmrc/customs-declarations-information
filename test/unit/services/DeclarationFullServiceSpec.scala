...
import uk.gov.hmrc.customs.declarations.information.connectors.{ApiSubscriptionFieldsConnector, DeclarationFullConnector, DeclarationVersionConnector, Non2xxResponseException}
...
class DeclarationFullServiceSpec extends UnitSpec with MockitoSugar with BeforeAndAfterEach{
  private val dateTime = new DateTime()
  private val headerCarrier: HeaderCarrier = HeaderCarrier()
  private implicit val vpr: AuthorisedRequest[AnyContentAsEmpty.type] = TestCspAuthorisedRequest

  protected lazy val mockFullResponseFilterService: FullResponseFilterService = mock[FullResponseFilterService]
  ...
  protected lazy val mockDeclarationFullConnector: DeclarationFullConnector = mock[DeclarationFullConnector]
  ...
  trait SetUp {
    ...
    protected lazy val service: DeclarationFullService = new DeclarationFullService(mockFullResponseFilterService, mockApiSubscriptionFieldsConnector,
      mockLogger, mockDeclarationFullConnector, mockDateTimeProvider, stubUniqueIdsService)
  ...
  "Business Service" should {
    ...
    "return 404 error response when backend call fails with 404" in new SetUp() {
      when(mockDeclarationFullConnector.send(any[DateTime],
        ...
        any[ApiVersion],
        any[Option[ApiSubscriptionFieldsResponse]],
        meq[Mrn](mrn))(any[AuthorisedRequest[_]])).thenReturn(Future.failed(new Non2xxResponseException(mock[HttpResponse], 404)))
      val result: Either[Result, HttpResponse] = send()
