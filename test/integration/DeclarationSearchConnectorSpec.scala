...
import uk.gov.hmrc.customs.declarations.information.connectors.{DeclarationSearchConnector, Non2xxResponseException}
...
class DeclarationSearchConnectorSpec extends IntegrationTestSpec
  ...
  private implicit val asr: AuthorisedRequest[AnyContent] = AuthorisedRequest(conversationId, VersionOne,
    ApiSubscriptionFieldsTestData.clientId, None, Some(searchParameters), Csp(Some(declarantEori), Some(badgeIdentifier)), mock[Request[AnyContent]])
