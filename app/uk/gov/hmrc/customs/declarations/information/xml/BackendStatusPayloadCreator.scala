...
@Singleton
class BackendStatusPayloadCreator() extends BackendPayloadCreator {
  def create[A](conversationId: ConversationId,
    ...
    val searchElement: Elem = <n1:labelToRename>{searchTypeAsType.toString}</n1:labelToRename>.copy(label = searchTypeAsType.label)
    val dateTimeFormatter: DateTimeFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'")
    val newLineAndIndentation: String = "\n        "
