...
@Singleton
class DeclarationVersionsService @Inject()(override val apiSubFieldsConnector: ApiSubscriptionFieldsConnector,
  ...
  private def filterResponse(response: HttpResponse, xmlResponseBody: Elem): HttpResponse = {
    HttpResponse(response.status, xmlResponseBody.toString(), response.headers)
  }
