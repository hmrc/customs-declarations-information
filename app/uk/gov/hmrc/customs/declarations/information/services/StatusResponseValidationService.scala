...
import uk.gov.hmrc.customs.declarations.information.model.BadgeIdentifier
...
class StatusResponseValidationService @Inject() (informationLogger: InformationLogger, informationConfigService: InformationConfigService) {
  ...
  private def validateAcceptanceDate(declarationNode: NodeSeq): Either[ErrorResponse, Boolean] =
    ...
    val isDateValid = parsedDateTime.fold(
      {
        informationLogger.errorWithoutRequestContext(s"Unable to parse acceptance date time: ${acceptanceDate.head}")
        false
      }
  ...
  private def validateCreationDate(declarationNode: NodeSeq): Either[ErrorResponse, Boolean] =
    extractField(declarationNode, "creationDate").fold[Either[ErrorResponse, Boolean]]({
      ...
      val isDateValid = parsedDateTime.fold(
        {
          informationLogger.errorWithoutRequestContext(s"Unable to parse creation date time: ${creationDate.head}")
          false
        }
      )(validDateTime => validDateTime.isAfter(getValidDateTimeUsingConfig))
      if (!isDateValid) {
