...
@Singleton
class VersionResponseFilterService @Inject()() {
  import uk.gov.hmrc.customs.declarations.information.xml.HelperXMLUtils._

  val NameSpaceP="http://gov.uk/customs/retrieveDeclarationVersion"
  ...
  def transform(xml: NodeSeq): NodeSeq = {
    ...
    val decVersionDetails: NodeSeq = xml \ "responseDetail" \ "RetrieveDeclarationVersionResponse" \ "RetrieveDeclarationVersionDetailsList" \\ "RetrieveDeclarationVersionDetails"
    ...
    xsi:schemaLocation={NameSpaceP}>
      {decVersionDetails.map{ node =>
        val declarations = node \ "Declaration"
        val mdgDeclaration = declarations.filter( node => inputPrefixToUriMap(node.prefix) == "http://gov.uk/customs/retrieveDeclarationVersion")
